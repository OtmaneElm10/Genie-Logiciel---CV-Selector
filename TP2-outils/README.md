...
#### Outillage : au moins une issue fermée dans GitLab [BAREME: 1]
...
### Outillage : les tests passent (mvn test -Dcheckstyle.skip) [BAREME: 1]
...
### Outillage : intégration continue OK sur GitLab [BAREME: 1]

Maintenant que vous avez corrigé les défauts et que vous avez envoyé une version
correcte via un `git push`, vous devriez voir sur la page d'accueil de votre
projet sur la forge un indicateur montrant que l'intégration continue passe
maintenant :

![commit: passed](img/commit-ok.png)

Vous devez avoir cet indicateur « passed » au moment du rendu final.
...
L'important ici est de spécifier la classe principale (`<mainClass>`). Pour
éviter de la répéter à plusieurs endroits du `pom.xml`, on l'a spécifiée une
bonne fois pour toutes dans la propriété `project.mainclass` en début de fichier :

```xml
    <properties>
        <!-- ... -->
        <project.mainclass>fr.univ_lyon1.info.m1.cv_search.App</project.mainclass>
    </properties>
```
...
### Outillage : absence d'erreur avec checkstyle [BAREME: 2]

Votre [projet](../projet-note.md) devra impérativement n'avoir aucun warning
avec checkstyle. Prenez l'habitude d'éliminer ces warnings au fur et à mesure
(via votre IDE et/ou `mvn test`, et en dernier recours l'intégration continue
qui vous dira si vous avez fait des bétises) : c'est très pénible de devoir tout
corriger après coup !
...
### Outillage : au moins une merge-request fermée sur GitLab [BAREME: 1]
...
[TP 3](../TP3-patterns), ne perdez pas de temps !
