...
Nous vous imposons la structure en package Java : un package `view` pour la vue, `model` pour le modèle, et `controller` pour le contrôleur.
...
#### Pattern : flexibilité du modèle MVC : deux vues correctement synchronisées [BAREME: 1]
...
quand des caractères sont saisis dans un champ texte, le texte entré n'est propagé
...
### Pattern : principes GRASP bien respectés [BAREME: 1]
...
### Pattern : Qualité et structure globale du code, utilisation de packages [BAREME: 1]

Votre code doit être bien organisé (toutes les bonnes pratiques vues depuis la licence), en particulier vos classes doivent être réparties en packages.

### Pattern : au moins 3 autres patterns (création, structure, SOLID, ...) que MVC ou GRASP [BAREME: 5]
...
Pendant le [TP1](../TP1-java/), vous avez dû faire ces modifications :

- Stratégie de sélection « tout >= 60% »
- Stratégie de sélection « moyenne >= 50% » 
- Note pour chaque candidat

Assurez-vous que ces modifications fonctionnent toujours et qu'elles sont codées
proprement en suivant le principe MVC.

### Fonctionnalité : au moins une autre stratégie de sélection de CV [BAREME: 1]
...
### Fonctionnalité : prise en compte de l'expérience professionnelle [BAREME: 2]
...
### Fonctionnalité : modification de l'interface (bouton pour supprimer une compétence de la liste) [BAREME: 1]
...
### Fonctionnalité : affichage de la liste avec affichage d'autre chose que le nom du candidat [BAREME: 1]
...
### Fonctionnalité : tri de la liste des candidats [BAREME: 1]

Triez la liste par note moyenne : le premier
candidat sera celui ayant la meilleure note.

### Fonctionnalité : autres extensions, meilleure interface, etc. [BAREME: 3]

Vous n'en avez pas assez ? Vous voulez ajouter un import de CV en PDF ? Un tri
par réseau de neurones ? Un import automatique depuis LinkedIN ? Une refonte de
l'IHM ?
...
Cette partie compte volontairement peu de points pour vous encourager à vous
concentrer sur la qualité des fonctions de base et non sur les améliorations
non-demandées.
